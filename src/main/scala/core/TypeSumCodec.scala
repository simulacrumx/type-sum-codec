...
class TypeSumCodec(typeHint: String = "type")
    extends scala.annotation.StaticAnnotation {
  def macroTransform(annottees: Any*): Any =
    macro TypeSumCodecMacros.innerMacros
}

private final class TypeSumCodecMacros(val c: blackbox.Context) {
  import c.universe._
  ...
  def innerMacros(annottees: Tree*): Tree = {
    annottees match {
      case List(
        ...
        val encoderCaseDefs = filtered.map { case typeName =>
        ...
        val decoderCaseDefs = filtered.map { case typeName =>
  ...
  private def filterChildrenCaseClasses(
  ...
  private[this] val codecTypeHintName: String = {
    c.prefix.tree match {
      ...
      case q"new ${`macroName`}(typeHint = $v)" =>
        v match {
